...
async function ingestAct(actName, actUrl, fallbackFilename) {
  ...
  try {
    ...
    console.warn(`Scraping failed for ${actName} (${err.message}). Using fallback file.`);
    try {
      actText = fs.readFileSync(path.join(FALLBACK_DIR, fallbackFilename), 'utf-8');
    } catch (fsErr) {
      console.error(`Fallback file not found: ${fallbackFilename}. Please manually download it.`);
      return;
    }
  }
  ...
  const chunks = actText.split(/(?=Section \d+)/i).filter(c => c.trim().length > 20);
  
  console.log(`Chunked ${actName} into ${chunks.length} sections. Embedding and inserting...`);
...
async function run() {
  ...
  await ingestAct('Transfer of Property Act, 1882', 'https://www.indiacode.nic.in/handle/123456789/2338', 'transfer_of_property_act.txt');
  await ingestAct('Indian Contract Act, 1872', 'https://www.indiacode.nic.in/handle/123456789/2187', 'indian_contract_act.txt');
  await ingestAct('Model Tenancy Act, 2021', 'https://mohua.gov.in/upload/uploadfiles/magazines/Model-Tenancy-Act.pdf', 'model_tenancy_act.txt');
