...
`;
  ...
  const clauses = [
    { id: 'clause_1', text: "CLAUSE 1: RENT & DEPOSIT\nThe monthly rent shall be Rs 25,000. The tenant shall deposit a security deposit of Rs 1,00,000." },
    { id: 'clause_2', text: "CLAUSE 2: SECURITY DEPOSIT FORFEITURE AND PENALTY\nIf the tenant terminates the lease early before 11 months, the entire security deposit of Rs 1,00,000 shall be 100% forfeited as penalty without any deduction or proof of actual loss." },
    { id: 'clause_3', text: "CLAUSE 3: MAINTENANCE & DAMAGE\nThe tenant is solely responsible for all structural repairs, building maintenance, and painting costs upon departure." }
  ];

  const freshSessionId = "fresh_voice_session_" + Date.now();
  const documentRisks = await ragService.analyzeDocumentLevelRisks(sampleDocumentText);
  ...
  const clauseDetailResult = await ragService.getClauseDetail(sessionStore[freshSessionId], "2");
  ...
  const hasOnlySection74 = voiceSourcesStore[freshSessionId].some(s => s.reference.includes("Section 74")) &&
                           !voiceSourcesStore[freshSessionId].some(s => s.reference.includes("Section 27") || s.reference.includes("Section 23"));

  if (hasOnlySection74) {
    console.log("SUCCESS: Sources stream cleanly isolated to Clause 2 & Section 74 only!");
  } else {
    console.warn("WARNING: Unexpected sources found in stream:", voiceSourcesStore[freshSessionId]);
  }
  ...
  const pureLawResult = await ragService.getLawCitation("Indian Contract Act", "74");
