...
async function createTestPDF(pdfPath) {
  return new Promise((resolve) => {
    ...
    doc
      .fontSize(12)
      .text(
        "1. The Tenant shall pay the monthly rent of Rs. 20,000 on or before the 5th day of every month.",
      );
    doc.moveDown();

    // Unfair clause 1 (Forfeiture of full deposit)
    doc.text(
      "2. In the event that the Tenant terminates this agreement before the expiry of the 11-month lock-in period, the Landlord shall forfeit and retain the entire security deposit of Rs. 1,00,000 as an absolute penalty, regardless of actual damages.",
    );
    doc.moveDown();

    // Unfair clause 2 (Unreasonable notice)
    doc.text(
      "3. The Landlord reserves the right to evict the Tenant and demand immediate vacant possession by providing only 2 days written notice, for any reason whatsoever.",
    );
    doc.moveDown();

    // Normal clause
    doc.text(
      "4. The Tenant shall be responsible for routine maintenance, including replacing lightbulbs and minor plumbing repairs, during the tenancy.",
    );
    doc.end();
  });
}
