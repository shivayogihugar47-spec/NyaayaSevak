...
router.post("/briefing", async (req, res) => {
  try {
    ...
    const defaultBriefing =
      "Hello! I am NyayaCheck, your legal assistant. I have reviewed your document. How can I help you today?";
...
router.post("/webhook", async (req, res) => {
  try {
    ...
    const sessionId =
      callMetadata.document_id || payload.document_id || message.document_id;
    ...
    if (!session || session.userId !== userId) {
      console.warn(
        `Webhook call rejected: Unauthorized access to document_id '${sessionId}'`,
      );
      ...
      return res.json({
        results: [
          {
            toolCallId:
              (message.toolCalls && message.toolCalls[0]?.id) || "error",
            result: JSON.stringify({
              error: "Document cannot be found or unauthorized access.",
            }),
          },
        ],
      });
      ...
      toolCalls = [
        {
          ...
          function: {
            ...
            arguments:
              message.functionCall.parameters || message.functionCall.arguments,
          },
        },
      ];
    ...
    for (const toolCall of toolCalls) {
      ...
      if (functionName === "get_document_risks") {
        toolResult = await ragService.getDocumentRisks(
          session,
          rawArgs.agreement_type || "unknown",
          rawArgs.max_risks || 5,
        );
      } else if (functionName === "get_clause_with_citations") {
        toolResult = await ragService.getClauseWithCitations(
          session,
          rawArgs.question,
          rawArgs.jurisdiction,
          rawArgs.clause_reference,
        );
