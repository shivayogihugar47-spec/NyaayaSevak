...
export default function DocumentViewer({ file, clauses, flags, activeClauseId, onClauseClick, chatClauseId, onBoxesReady }) {
  ...
  useEffect(() => {
    const findTextBounds = () => {
      ...
      if (isImage) {
        ...
        const marginX = rect.width * 0.10; // 10% horizontal margin
        ...
        clauses.forEach(clause => {
          ...
          newBoxes[clause.id] = {
            top: rect.top - containerRect.top + containerRef.current.scrollTop + marginY + currentTopOffset,
        ...
        clauses.forEach(clause => {
          ...
          for (let span of spans) {
            if (span.textContent.includes(firstWords) || firstWords.includes(span.textContent)) {
              foundSpan = span;
              break;
            }
          }
          ...
          if (foundSpan) {
            ...
            newBoxes[clause.id] = {
              ...
              width: containerRect.width - (rect.left - containerRect.left) - 40, 
              height: 60,
              absTop: rect.top,
              absLeft: rect.left,
            };
  ...
  return (
    ...
    >
      <div className="sticky top-0 left-0 bg-[#0a0a0c]/90 backdrop-blur-xl py-4 px-6 z-10 flex justify-between border-b border-white/5 shadow-sm">
        ...
        {numPages && (
          <div className="flex gap-3 items-center">
             <button onClick={() => setPageNumber(Math.max(1, pageNumber-1))} className="text-xs bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg transition-colors font-medium">Prev</button>
             <span className="text-xs font-medium text-white/70">Page {pageNumber} of {numPages}</span>
             <button onClick={() => setPageNumber(Math.min(numPages, pageNumber+1))} className="text-xs bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-lg transition-colors font-medium">Next</button>
          </div>
        )}
      </div>
      ...
      <div className="relative inline-block mx-auto min-w-full p-4">
        {isImage ? (
          <img src={URL.createObjectURL(file)} alt="Document" className="w-full max-w-2xl mx-auto" />
        ) : file ? (
          <Document
            file={file}
          ...
          <div className="text-neutral-500 p-10 text-center">No visual preview available.</div>
        )}
        ...
        {Object.entries(boxes).map(([id, box]) => {
          ...
          let colorClass = "border-dashed border-white/10 hover:border-indigo-400/50 hover:bg-indigo-500/5 transition-all duration-300"; // unanalyzed
          if (analysis) {
             if (analysis.risk_level === 'High') colorClass = "bg-gradient-to-br from-red-500/20 to-red-600/10 border border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.2)] animate-pulse";
             else if (analysis.risk_level === 'Medium') colorClass = "bg-gradient-to-br from-yellow-500/20 to-yellow-600/10 border border-yellow-500/50 shadow-[0_0_15px_rgba(234,179,8,0.2)]";
             else colorClass = "bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 border border-emerald-500/30";
          }
