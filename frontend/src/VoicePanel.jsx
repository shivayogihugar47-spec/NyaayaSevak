...
function VoicePanel({ sessionId, onClose }) {
  ...
  const startVoiceCall = async () => {
    try {
      ...
      if (!VAPI_PUBLIC_KEY) {
        throw new Error(
          "VITE_VAPI_PUBLIC_KEY environment variable is missing.",
        );
      }
      if (!VAPI_ASSISTANT_ID) {
        throw new Error(
          "VITE_VAPI_ASSISTANT_ID environment variable is missing.",
        );
      }
      if (!sessionId) {
      ...
      if (typeof VapiClass !== "function") {
        throw new Error(
          "Could not resolve Vapi constructor from @vapi-ai/web SDK.",
        );
      }
      ...
      vapi.on("message", (message) => {
        ...
        if (message.type === "function-call" || message.type === "tool-calls") {
          axios
            .get(`/api/voice/sources/${sessionId}`, { withCredentials: true })
            .then((res) => {
              if (res.data?.sources) setSources(res.data.sources);
            })
            .catch(() => {});
        ...
        if (message.type === "status-update" && message.status === "ended") {
          if (
            message.endedReason &&
            message.endedReason.includes("error-vapifault")
          ) {
            setErrorMsg(
              `Voice call disconnected: Transcriber error (${message.endedReason}).`,
            );
          }
        }
      });
      ...
      vapi.on("error", (err) => {
        ...
        if (rawString.includes("401") || rawString.includes("Unauthorized")) {
          setErrorMsg(
            "Vapi Public Key Unauthorized (401). Please verify VITE_VAPI_PUBLIC_KEY in frontend/.env.",
          );
        } else if (
          rawString.includes("400") ||
          rawString.includes("Bad Request")
        ) {
      ...
      setErrorMsg(
        "Failed to start voice call: " + (err.message || "Unknown error"),
      );
      setCallState("idle");
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-400">
                Proactive briefing & real-time statutory function calling
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="bg-gradient-to-b from-indigo-950/30 to-black/40 border border-indigo-500/20 rounded-2xl p-6 text-center flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <div className="relative mb-6">
              ...
              >
                {callState === "connecting" ? (
                  ...
                  <Volume2
                    className={`w-10 h-10 ${isSpeaking ? "text-indigo-300 animate-bounce" : "text-green-400"}`}
                  />
                ) : (
            ...
            <div className="text-sm font-semibold mb-3 tracking-wide uppercase">
              ...
              {callState === "ended" && (
                <span className="text-neutral-500">Call Ended</span>
              )}
            </div>
            ...
            {briefing && (
              <div className="bg-black/50 border border-white/10 rounded-xl p-4 text-left max-w-lg w-full">
                <div className="text-[11px] font-bold text-indigo-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5" /> Proactive Opening
                  Briefing
                </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-neutral-500">
                {sources.length} Active Citations
              </span>
            </div>

            {sources.length === 0 ? (
              <div className="bg-white/[0.02] border border-white/5 rounded-xl p-4 text-center text-xs text-neutral-500">
                Sources will appear here in real time as statutory function
                calls execute during speech.
              </div>
            ) : (
            ...
            <div>
              ...
              <div className="space-y-2.5 max-h-40 overflow-y-auto bg-black/40 border border-white/5 rounded-xl p-4 text-xs">
                {transcript.map((msg, i) => (
                  <div
                    key={i}
                    className={`flex gap-2 ${msg.role === "user" ? "text-indigo-300" : "text-neutral-300"}`}
                  >
                    <span className="font-bold uppercase text-[10px] opacity-60 w-16 shrink-0">
                      {msg.role}:
                    </span>
                    <span>{msg.text}</span>
                  </div>
